    ...
    <div>
        ...
        <app-body>
            <div style="min-height: 85vh;">
                ...
                <div style="margin: 0 20px;" id="RecommendIdle">
                <h3 style="color: dodgerblue">推荐商品</h3>
                <el-row :gutter="30">
                    <el-col :span="6" :key="idle.id" v-for="(idle,index) in RecommendIdleList">
            ...
            <div style="margin: 0 20px;" id="AllIdle">
                <div><h3 style="color: dodgerblue">全部商品</h3></div>
                ...
                <el-row :gutter="30">
                    <el-col :span="6" :key="idle.id" v-for="(idle,index) in idleList">
                        <div class="idle-card" @click="toDetails(idle)">
                            <el-image
                                    style="width: 100%; height: 160px"
                                    :src="idle.imgUrl"
    ...
    export default {
        ...
        watch:{
            $route(to,from){
                ...
                this.findRecommendIdleTiem(val);
            }
        },
        methods: {
            findRecommendIdleTiem(page){
                ...
                this.$api.findRecommendIdleTiem({
                        page: page,
                        nums: 8
                    }).then(res => {
                        console.log(res);
                        ...
                        this.RecommendIdleList = list;
                        this.totalRecommendItem=res.data.count;
                        console.log(this.totalRecommendItem);
                    }).catch(e => {
                        console.log(e)
                    }).finally(()=>{
                        loading.close();
            ...
            toDetails(idle) {
                ...
                console.log('toDetails called with idle:', idle);
                ...
                // 获取到用户ID后，继续进行统计点击和跳转到详情页的操作
                this.RecordHits(idle);
                
                //前往详情页
                this.$router.push({path: '/details', query: {id: idle.id}});
            },
            RecordHits(idle) {
                console.log('RecordHits called with idle.id:', idle.id);
                // 使用 this.$api 发送点击事件到后端
                this.$api.recordClick(
                    {productId: idle.id}
                    )
                .then(response => {
                    console.log('RecordHits 响应:', response);
                    // 处理响应，如果需要的话
                    // 可以刷新商品列表或其他操作
                })
                .catch(error => {
